...
class layerfx_base(object):
  ...
  def add_under_layer(self, newlayer, oldlayer):
    img = pdb.gimp_item_get_image(oldlayer)
    if pdb.gimp_item_get_parent(oldlayer) == None:
      pdb.gimp_image_insert_layer(img, newlayer, pdb.gimp_item_get_parent(oldlayer), pdb.gimp_image_get_item_position(img, oldlayer) + 1)
    else:
  ...
  def add_over_layer(self, newlayer, oldlayer):
    img = pdb.gimp_item_get_image(oldlayer)
    if pdb.gimp_item_get_parent(oldlayer) == None:
      pdb.gimp_image_insert_layer(img, newlayer, pdb.gimp_item_get_parent(oldlayer), pdb.gimp_image_get_item_position(img, oldlayer))
    else:
      self.img.add_layer(newlayer, self.get_layer_pos(oldlayer))

  def group_layers(self, name, *layers):
    img = pdb.gimp_item_get_image(layers[0])
    lg = pdb.gimp_layer_group_new(img)
    self.add_over_layer(lg, layers[0])
    pdb.gimp_image_reorder_item(img, lg, pdb.gimp_item_get_parent(layers[0]), pdb.gimp_image_get_item_position(img, layers[0]))
    pdb.gimp_item_set_name(lg, name)
    for i, l in enumerate(layers):
      pdb.gimp_image_reorder_item(img, l, lg, i)
  ...
  def make_blend_mode_box(self):
    return gimpui.IntComboBox((
      "Normal",        NORMAL_MODE,
      "Dissoudre",      DISSOLVE_MODE,
      "Multiplier",      MULTIPLY_MODE,
      "Diviser",        DIVIDE_MODE,
      "Ecran",        SCREEN_MODE,
      "Superposer",       OVERLAY_MODE,
      "Eclaircir",         DODGE_MODE,
      "Assombrir",          BURN_MODE,
      "Lumiere dure",    HARDLIGHT_MODE,
      "Lumiere douce",    SOFTLIGHT_MODE,
      "Extraction de grain", GRAIN_EXTRACT_MODE,
      "Fusion de grain",   GRAIN_MERGE_MODE,
      "Difference",    DIFFERENCE_MODE,
      "Addition",      ADDITION_MODE,
      "Soustrait",      SUBTRACT_MODE,
      "Assombrir seulement",   DARKEN_ONLY_MODE,
      "Eclaircir seulement",  LIGHTEN_ONLY_MODE,
      "Teinte",           HUE_MODE,
  ...
  def make_contour_box(self):
    return self.make_combo_box(
      ...
      "Cone",
      "Cone inverse",
      "Denivellement profond",
      "Denivellement leger",
      "Gaussien",
      "Demi-Cercle",
      "Rouleau",
      "Double rouleau",
      "Pente accidentee descendante",
      "Palliers arrondis",
      "Dents de scie 1"
    )
  ...
  def make_gradient_repeat_box(self):
    return self.make_combo_box("Non", "Dents de scie", "Onde triangulaire")

  def make_interpolation_box(self):
    return self.make_combo_box("Non", "Lineaire", "Cubique", "Sinc (Lanczos3)")
...
class layerfx_drop_shadow(layerfx_base):
  ...
  def __init__(self, runmode, img, drawable, color, opacity, contour, noise, mode, spread, size, offsetangle, offsetdist, knockout, merge):
    ...
    if runmode == RUN_INTERACTIVE:
      ...
      if self.validatedata(img, drawable,
        ...
        self.removeOldLayer()
        fxlayer = self.makeShadow(img, drawable, color, opacity, contour, noise, mode, spread, size, offsetangle, offsetdist, knockout, merge, 0)
        if merge == 0:
  ...
  def showDialog(self):
    self.dialog = gimpui.Dialog("Ombre portee", "dropshadowdialog")
    ...
    self.color_label = self.make_label("_Couleur:")
    ...
    self.mode_label = self.make_label("_Mode de fusion:")
    ...
    self.opacity_label = self.make_label("_Opacite:")
    ...
    self.spread_label = self.make_label("_Grossi:")
    ...
    self.size_label = self.make_label("T_aille:")
    ...
    self.noise_label = self.make_label("_Bruit:")
    ...
    self.knockout_check = gtk.CheckButton("Ombre portee masquee par le calque")
    if self.parasitedata:
    ...
    self.merge_check = gtk.CheckButton("_Fusionner avec le calque")
    if self.parasitedata:
    ...
    self.preview_check = gtk.CheckButton("_Apercu")
  ...
  def makeShadow(self, img, drawable, color, opacity, contour, noise, mode, spread, size, offsetangle, offsetdist, knockout, merge, preview):
    ...
    if merge == 0 and preview == 0:
      self.group_layers("%s-with-dropshadow" % (drawable.name), drawable, shadowlayer)
...
class layerfx_inner_shadow(layerfx_base):
  ...
  def __init__(self, runmode, img, drawable, color, opacity, contour, noise, mode, source, choke, size, offsetangle, offsetdist, merge):
    ...
    if runmode == RUN_INTERACTIVE:
      ...
      if self.validatedata(img, drawable,
        ...
        self.removeOldLayer()
        fxlayer = self.makeShadow(img, drawable, color, opacity, contour, noise, mode, source, choke, size, offsetangle, offsetdist, merge, 0)
        if merge == 0:
  ...
  def showDialog(self):
    self.dialog = gimpui.Dialog("Ombre interne", "innershadowdialog")
    ...
    self.source_center_radio = gtk.RadioButton(None, "Cent_re", True)
    self.source_edge_radio = gtk.RadioButton(self.source_center_radio, "Contour", True)
    if self.parasitedata:
    ...
    self.choke_label = self.make_label("M_aigri:")
  ...
  def makeShadow(self, img, drawable, color, opacity, contour, noise, mode, source, choke, size, offsetangle, offsetdist, merge, preview):
    pdb.gimp_image_undo_group_start(img)
    origfgcolor = gimp.get_foreground()
    origselection = pdb.gimp_selection_save(img)
    growamt = int(math.ceil(size / 2.0))
    ...
    if merge == 0 and preview == 0:
      self.group_layers("%s-with-innershadow" % (drawable.name), shadowlayer, drawable)
...
class layerfx_outer_glow(layerfx_base):
  ...
  def __init__(self, runmode, img, drawable, color, opacity, contour, noise, mode, spread, size, knockout, merge):
    ...
    if runmode == RUN_INTERACTIVE:
      ...
      if self.validatedata(img, drawable,
        ...
        self.removeOldLayer()
        fxlayer = self.makeGlow(img, drawable, color, opacity, contour, noise, mode, spread, size, knockout, merge, 0)
        if merge == 0:
  ...
  def showDialog(self):
    self.dialog = gimpui.Dialog("Lueur externe", "outerglowdialog")
    ...
    self.color_label = self.make_label("Couleur:")
    self.table.attach(self.color_label, 0, 1, 0, 1)
    ...
    self.knockout_check = gtk.CheckButton("Lueur externe masquee par le calque")
    if self.parasitedata:
      if self.parasitedata["knockout"] == 1:
        self.knockout_check.set_active(True)
    elif shelf.has_key(self.shelfkey) == 1 and shelf[self.shelfkey]["knockout"] == 1:
  ...
  def makeGlow(self, img, drawable, color, opacity, contour, noise, mode, spread, size, knockout, merge, preview):
    pdb.gimp_image_undo_group_start(img)
    origfgcolor = gimp.get_foreground()
    ...
    if merge == 0 and preview == 0:
      self.group_layers("%s-with-outerglow" % (drawable.name), drawable, glowlayer)
...
class layerfx_inner_glow(layerfx_base):
  ...
  def __init__(self, runmode, img, drawable, color, opacity, contour, noise, mode, source, choke, size, merge):
    ...
    if runmode == RUN_INTERACTIVE:
      ...
      if self.validatedata(img, drawable,
        ...
        self.removeOldLayer()
        fxlayer = self.makeGlow(img, drawable, color, opacity, contour, noise, mode, source, choke, size, merge, 0)
        if merge == 0:
          if type(color) == gimpcolor.RGB:
  ...
  def showDialog(self):
    self.dialog = gimpui.Dialog("Lueur interne", "innerglowdialog")
    ...
    self.choke_label = self.make_label("C_ontour:")
    ...
    self.noise_label = self.make_label("Bruit:")
  ...
  def makeGlow(self, img, drawable, color, opacity, contour, noise, mode, source, choke, size, merge, preview):
    pdb.gimp_image_undo_group_start(img)
    origfgcolor = gimp.get_foreground()
    origgradient = pdb.gimp_context_get_gradient()
    origselection = pdb.gimp_selection_save(img)
    ...
    if merge == 0 and preview == 0:
      self.group_layers("%s-with-innerglow" % (drawable.name), glowlayer, drawable)
...
class layerfx_bevel_emboss(layerfx_base):
  ...
  def __init__(self, runmode, img, drawable, style, depth, direction, size, soften, angle, altitude, glosscontour, highlightcolor, highlightmode, highlightopacity, shadowcolor, shadowmode, shadowopacity, surfacecontour, use_texture, pattern, scale, tex_depth, invert, merge):
    ...
    if runmode == RUN_INTERACTIVE:
      ...
      if self.validatedata(img, drawable,
        ...
        self.removeOldLayer()
        fxlayer = self.makeBevel(img, drawable, style, depth, direction, size, soften, angle, altitude, glosscontour, highlightcolor, highlightmode, highlightopacity, shadowcolor, shadowmode, shadowopacity, surfacecontour, use_texture, pattern, scale, tex_depth, invert, merge, 0)
        if merge == 0:
  ...
  def showDialog(self):
    self.dialog = gimpui.Dialog("Biseautage et Estampage", "bevelembossdialog")
    ...
    self.style_box = self.make_combo_box("Biseau Externe", "Biseau Interne", "Estampage", "Estampage Oreiller (biseau interne inverse)")
    if self.parasitedata:
    ...
    self.depth_label = self.make_label("_Profondeur:")
    ...
    self.direction_up_radio = gtk.RadioButton(None, "Haut", True)
    self.direction_down_radio = gtk.RadioButton(self.direction_up_radio, "Bas", True)
    if self.parasitedata:
    ...
    self.soften_label = self.make_label("F_lou:")
    ...
    self.surfacecontour_label = self.make_label("Contour de la Surface:")
    ...
    self.shadeframe = gimpui.Frame("Ombrage")
    ...
    self.altitude_label = self.make_label("_Elevation:")
    ...
    self.glosscontour_label = self.make_label("Contour Brillant:")
    ...
    self.highlightmode_label = self.make_label("Mode des tons clairs:")
    ...
    self.highlightcolor_label = self.make_label("Couleur:")
    ...
    self.highlightopacity_label = self.make_label("Opacite des tons clairs:")
    ...
    self.shadowmode_label = self.make_label("Mode des tons fonces:")
    ...
    self.shadowcolor_label = self.make_label("Couleur:")
    ...
    self.shadowopacity_label = self.make_label("Opacite des tons fonces:")
    ...
    self.use_texture_check = gtk.CheckButton("Utiliser Texture")
    if self.parasitedata:
    ...
    self.pattern_label = self.make_label("_Motif:")
    ...
    self.invert_check = gtk.CheckButton("_Inverser")
    if self.parasitedata:
    ...
    self.scale_label = self.make_label("Echelle:")
    ...
    self.tex_depth_label = self.make_label("Profondeur:")
  ...
  def makeBevel(self, img, drawable, style, depth, direction, size, soften, angle, altitude, glosscontour, highlightcolor, highlightmode, highlightopacity, shadowcolor, shadowmode, shadowopacity, surfacecontour, use_texture, pattern, scale, tex_depth, invert, merge, preview):
    pdb.gimp_image_undo_group_start(img)
    origfgcolor = gimp.get_foreground()
    origselection = pdb.gimp_selection_save(img)
    ...
    if merge == 0 and preview == 0:
      self.group_layers("%s-with-bevel" % (drawable.name), highlightlayer, shadowlayer, drawable)
    if merge == 1:
      if style == 1:
        origmask = drawable.mask
        layername = drawable.name
...
class layerfx_satin(layerfx_base):
  ...
  def __init__(self, runmode, img, drawable, color, opacity, mode, offsetangle, offsetdist, size, contour, invert, merge):
    ...
    if runmode == RUN_INTERACTIVE:
      ...
      if self.validatedata(img, drawable,
        ...
        self.removeOldLayer()
        fxlayer = self.makeSatin(img, drawable, color, opacity, mode, offsetangle, offsetdist, size, contour, invert, merge, 0)
        if merge == 0:
  ...
  def showDialog(self):
    ...
    self.invert_check = gtk.CheckButton("Inverser")
    if self.parasitedata:
  ...
  def makeSatin(self, img, drawable, color, opacity, mode, offsetangle, offsetdist, size, contour, invert, merge, preview):
    pdb.gimp_image_undo_group_start(img)
    origfgcolor = gimp.get_foreground()
    origselection = pdb.gimp_selection_save(img)
    growamt = int(math.ceil(size / 2.0))
    ...
    if merge == 0 and preview == 0:
      self.group_layers("%s-with-satin" % (drawable.name), satinlayer, drawable)
    if contour > 0:
...
class layerfx_stroke(layerfx_base):
  ...
  def __init__(self, runmode, img, drawable, fill, opacity, mode, size, position, merge):
    ...
    if runmode == RUN_INTERACTIVE:
      ...
      if self.validatedata(img, drawable,
        ...
        self.removeOldLayer()
        fxlayer = self.makeStroke(img, drawable, fill, opacity, mode, size, position, merge, 0)
  ...
  def showDialog(self):
    self.dialog = gimpui.Dialog("Contour", "strokedialog")
    ...
    self.position_label2 = gtk.Label("0 = interieur, 100 = exterieur")
    ...
    self.fill_type_frame = gimpui.Frame("Remplissage")
    ...
    self.fill_type_color_radio = gtk.RadioButton(None, "Couleur", True)
    self.fill_type_gradient_radio = gtk.RadioButton(self.fill_type_color_radio, "Degrade", True)
    self.fill_type_pattern_radio = gtk.RadioButton(self.fill_type_color_radio, "Motif", True)
    ...
    self.gradient_label = self.make_label("_Degrade:", False)
    ...
    self.gradient_type_label = self.make_label("Type:", False)
    ...
    self.gradient_repeat_label = self.make_label("Repeter:", False)
    ...
    self.gradient_reverse_check = gtk.CheckButton("_Inverser")
    if self.parasitedata:
    ...
    self.gradient_center_label = self.make_label("_Centre:", False)
    ...
    self.gradient_angle_label = self.make_label("_Angle:", False)
    ...
    self.gradient_width_label = self.make_label("_Echelle:", False)
    ...
    self.pattern_label = self.make_label("_Motif:", False)
    ...
    self.pattern_scale_label = self.make_label("Echelle:", False)
  ...
  def makeStroke(self, img, drawable, fill, opacity, mode, size, position, merge, preview):
    pdb.gimp_image_undo_group_start(img)
    origselection = pdb.gimp_selection_save(img)
    if position == 0:
      ...
      if merge == 0 and preview == 0:
        self.group_layers("%s-with-stroke" % (drawable.name), strokelayer, drawable)
      ...
      if merge == 0 and preview == 0:
        self.group_layers("%s-with-stroke" % (drawable.name), drawable, strokelayer)
...
class layerfx_color_overlay(layerfx_base):
  ...
  def __init__(self, runmode, img, drawable, color, opacity, mode, merge):
    ...
    if runmode == RUN_INTERACTIVE:
      ...
      if self.validatedata(img, drawable,
        ...
        self.removeOldLayer()
        fxlayer = self.makeOverlay(img, drawable, color, opacity, mode, merge, 0)
        if merge == 0:
  ...
  def showDialog(self):
    self.dialog = gimpui.Dialog("Incrustation couleur", "coloroverlaydialog")
  ...
  def makeOverlay(self, img, drawable, color, opacity, mode, merge, preview):
    pdb.gimp_image_undo_group_start(img)
    origfgcolor = gimp.get_foreground()
    origselection = pdb.gimp_selection_save(img)
    ...
    if merge == 0 and preview == 0:
      self.group_layers("%s-with-color" % (drawable.name), colorlayer, drawable)
...
class layerfx_gradient_overlay(layerfx_base):
  ...
  def __init__(self, runmode, img, drawable, gradient, gradienttype, repeat, reverse, opacity, mode, centerx, centery, angle, width, merge):
    ...
    if runmode == RUN_INTERACTIVE:
      ...
      if self.validatedata(img, drawable,
        ...
        self.removeOldLayer()
        fxlayer = self.makeOverlay(img, drawable, gradient, gradienttype, repeat, reverse, opacity, mode, centerx, centery, angle, width, merge, 0)
        if merge == 0:
  ...
  def showDialog(self):
    self.dialog = gimpui.Dialog("Incrustation Degrade", "gradientoverlaydialog")
    ...
    self.gradient_label = self.make_label("_Degrade:")
    ...
    self.gradienttype_label = self.make_label("Type:")
    ...
    self.repeat_label = self.make_label("Repeter:")
    ...
    self.reverse_check = gtk.CheckButton("_Inverser")
    if self.parasitedata:
      if self.parasitedata["reverse"] == 1:
    ...
    self.center_label = self.make_label("_Centre:")
    ...
    self.angle_label = self.make_label(" Angle:")
    ...
    self.width_label = self.make_label("_Echelle:")
  ...
  def makeOverlay(self, img, drawable, gradient, gradienttype, repeat, reverse, opacity, mode, centerx, centery, angle, width, merge, preview):
    pdb.gimp_image_undo_group_start(img)
    origgradient = pdb.gimp_context_get_gradient()
    origselection = pdb.gimp_selection_save(img)
    ...
    if merge == 0 and preview == 0:
      self.group_layers("%s-with-gradient" % (drawable.name), gradientlayer, drawable)
...
class layerfx_pattern_overlay(layerfx_base):
  ...
  def __init__(self, runmode, img, drawable, pattern, opacity, mode, scale, interpolation_type, merge):
    ...
    if runmode == RUN_INTERACTIVE:
      ...
      if self.validatedata(img, drawable,
        ...
        self.removeOldLayer()
        fxlayer = self.makeOverlay(img, drawable, pattern, opacity, mode, scale, interpolation_type, merge, 0)
        if merge == 0:
  ...
  def showDialog(self):
    self.dialog = gimpui.Dialog("Incrustation motif", "patternoverlaydialog")
  ...
  def makeOverlay(self, img, drawable, pattern, opacity, mode, scale, interpolation_type, merge, preview):
    pdb.gimp_image_undo_group_start(img)
    origpattern = pdb.gimp_context_get_pattern()
    origselection = pdb.gimp_selection_save(img)
    if scale == 100.0:
    ...
    if merge == 0 and preview == 0:
      self.group_layers("%s-with-pattern" % (drawable.name), patternlayer, drawable)
...
class layerfxplugin(gimpplugin.plugin):
  ...
  def query(self):
    ...
    imgmenupath = "<Image>/Filters/Styles de calque (Python)/"
    layersmenupath = "<Filters>/_Styles de calque (Python)/"
    date = "July 2008"
    ...
    drop_shadow_description = "Ajouter une ombre portee a un calque."
    drop_shadow_help = "Ajouter une ombre portee a un calque."
    drop_shadow_params = (
    ...
    gimp.install_procedure(
      ...
      copyrightname,
      date,
      "%s_Ombre portee..." % (layersmenupath),
      "RGBA, GRAYA",
      PLUGIN,
      drop_shadow_params,
      []
    )
    inner_shadow_description = "Ajouter une ombre interne a un calque"
    inner_shadow_help = "Ajouter une ombre interne a un calque"
    inner_shadow_params = (
      (PDB_INT32,    "run_mode",        "Run mode"),
      (PDB_IMAGE,    "image",           "Input image"),
      (PDB_DRAWABLE, "drawable",        "Input drawable"),
      ...
      (PDB_FLOAT,    "noise",           "The amount of noise applied to the shadow (0 <= noise <= 100)"),
      (PDB_INT32,    "mode",            "The shadow layer's combination mode %s" % (modevals)),
      (PDB_INT32,    "source",          "Source (0 = Centre, 1 = Contour)"),
    ...
    gimp.install_procedure(
      ...
      copyrightname,
      date,
      "%s_mbre Interne..." % (layersmenupath),
      "RGB*, GRAY*",
      PLUGIN,
      inner_shadow_params,
      []
    )
    outer_glow_description = "Creer une lueur externe autour d'un calque."
    outer_glow_help = "Creer une lueur externe autour d'un calque."
    outer_glow_params = (
      (PDB_INT32,    "run_mode", "Run mode"),
      (PDB_IMAGE,    "image",    "Input image"),
      (PDB_DRAWABLE, "drawable", "Input drawable"),
    ...
    gimp.install_procedure(
      ...
      copyrightname,
      date,
      "%s_Lueur Externe..." % (layersmenupath),
      "RGBA, GRAYA",
      PLUGIN,
      outer_glow_params,
      []
    )
    inner_glow_description = "Ajouter un effet de lueur interne sur un calque."
    inner_glow_help = "Ajouter un effet de lueur interne sur un calque."
    inner_glow_params = (
      (PDB_INT32,    "run_mode", "Run mode"),
      (PDB_IMAGE,    "image",    "Input image"),
      (PDB_DRAWABLE, "drawable", "Input drawable"),
    ...
    gimp.install_procedure(
      ...
      copyrightname,
      date,
      "%s_Lueur Interne..." % (layersmenupath),
      "RGB*, GRAY*",
      PLUGIN,
      inner_glow_params,
      []
    )
    bevel_emboss_description = "Creer des effets de biseautage et d'estampage sur un calque."
    bevel_emboss_help = "Creer des effets de biseautage et d'estampage sur un calque."
    bevel_emboss_params = (
      (PDB_INT32,    "run_mode",          "Run mode"),
      (PDB_IMAGE,    "image",             "Input image"),
      (PDB_DRAWABLE, "drawable",          "Input drawable"),
    ...
    gimp.install_procedure(
      ...
      copyrightname,
      date,
      "%s_Biseautage et Estampage..." % (imgmenupath),
      "RGBA, GRAYA",
      PLUGIN,
      bevel_emboss_params,
      []
    ...
    gimp.install_procedure(
      ...
      copyrightname,
      date,
      "%s_Biseautage et Estampage..." % (layersmenupath),
      "RGBA, GRAYA",
      PLUGIN,
      bevel_emboss_params,
      []
    )
    satin_description = "Creer un effet satine sur un calque."
    satin_help = "Creer un effet satine sur un calque."
    satin_params = (
      (PDB_INT32,    "run_mode",        "Run mode"),
      (PDB_IMAGE,    "image",           "Input image"),
      (PDB_DRAWABLE, "drawable",        "Input drawable"),
    ...
    stroke_description = "Appliquer un contour autour d'un calque."
    stroke_help = "Appliquer un contour autour d'un calque."
    stroke_params = (
      (PDB_INT32,    "run_mode", "Run mode"),
      (PDB_IMAGE,    "image",    "Input image"),
      (PDB_DRAWABLE, "drawable", "Input drawable"),
    ...
    color_overlay_description = "Incruster une couleur sur un calque."
    color_overlay_help = "Incruster une couleur sur un calque."
    ...
    gimp.install_procedure(
      ...
      copyrightname,
      date,
      "%s_Incrustation Couleur..." % (imgmenupath),
      "RGB*, GRAY*",
      PLUGIN,
      color_overlay_params,
      []
    ...
    gimp.install_procedure(
      ...
      copyrightname,
      date,
      "%s_Incrustation Couleur..." % (layersmenupath),
      "RGB*, GRAY*",
      PLUGIN,
      color_overlay_params,
      []
    )
    gradient_overlay_description = "Incruster un degrade sur un calque."
    ...
    gimp.install_procedure(
      ...
      copyrightname,
      date,
      "%s_Incrustation Degrade..." % (imgmenupath),
      "RGB*, GRAY*",
      PLUGIN,
      gradient_overlay_params,
      []
    ...
    gimp.install_procedure(
      ...
      copyrightname,
      date,
      "%s_Incrustation Degrade..." % (layersmenupath),
      "RGB*, GRAY*",
      PLUGIN,
      gradient_overlay_params,
      []
    )
    pattern_overlay_description = "Incruster un motif sur un calque."
    pattern_overlay_help = "Incruster un motif sur un calque."
    ...
    gimp.install_procedure(
      ...
      copyrightname,
      date,
      "%s_Incrustation Motif..." % (imgmenupath),
      "RGB*, GRAY*",
      PLUGIN,
      pattern_overlay_params,
      []
    ...
    gimp.install_procedure(
      ...
      copyrightname,
      date,
      "%s_Incrustation Motif..." % (layersmenupath),
      "RGB*, GRAY*",
      PLUGIN,
      pattern_overlay_params,
      []
    )
    reapply_effects_description = "Re-appliquer tous les effets precedemment appliques a un calque."
    reapply_effects_help = "Re-appliquer tous les effets precedemment appliques a un calque. NOTE: Ne fonctionne pas pour les effets qui ont ete appliques avec l'option \"Fusionner avec le calque\" active."
    ...
    gimp.install_procedure(
      ...
      copyrightname,
      date,
      "%s_Re-appliquer l'effet" % (imgmenupath),
      "RGB*, GRAY*",
      PLUGIN,
      reapply_effects_params,
      []
